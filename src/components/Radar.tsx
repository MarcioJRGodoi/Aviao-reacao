...
// Linha e coluna centrais do radar
const Line = styled.div<{ vertical?: boolean }>`
  position: absolute;
  background-color: black;
  ${(props) => (props.vertical ? 'width: 2px; height: 600px; left: 299px;' : 'height: 2px; width: 600px; top: 299px;')}
`;
...
const Radar: React.FC = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => setIsInitial(!isInitial), 3000);
  ...
  return (
    <RadarContainer isInitial={isInitial}>
      <div style={{ width: '600px', height: '600px', position: 'relative' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(10, 60px)', width: '600px' }}>
...
<div
            // biome-ignore lint/suspicious/noArrayIndexKey: <explanation>
            key={index}
            onClick={() => alert(`Selected plane at (${plane.x}, ${plane.y})`)}
            style={{
              position: 'absolute',
              top: `${fixY(plane.y)}px`,
